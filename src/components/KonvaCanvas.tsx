...
const KonvaCanvas = forwardRef<unknown, KonvaCanvasProps>(
  (
    {
      imageUrl,
      imageWidth,
    ...
    >("loading");
    const textRefs = useRef<{ [key: string]: unknown }>({});
      ...
      if (transformerRef.current) {
        try {
          if (selectedIds.size > 0) {
            ...
            const hasUnlockedLayers = selectedLayers.some(
              (layer) => !layer.locked
            );
    ...
    const renderSpacingHints = () => {
      const layersWithHints = textLayers.filter(
        (layer) => layer.showSpacingHints
      );
      ...
      const getPositionAndRotation = (index: number, totalChars: number) => {
        ...
        switch (pathType) {
          ...
          case "wave":
            ...
            const waveX =
              centerX +
              (waveIndex - waveCount / 2) * ((radius * 2) / waveCount);
            const nextWaveX =
              centerX +
              (waveIndex + 1 - waveCount / 2) * ((radius * 2) / waveCount);
          ...
          case "zigzag":
            ...
            y =
              centerY +
              (zigzagIndex % 2 === 0 ? -zigzagHeight : zigzagHeight) *
                (1 - zigzagFraction);
            rotation = 0;
            break;
          ...
          case "heart":
            ...
            rotation =
              (Math.atan2(Math.cos(heartAngle), -Math.sin(heartAngle)) * 180) /
              Math.PI;
            break;
          ...
          case "star":
            ...
            const starIndex = Math.floor(
              starAngle / ((2 * Math.PI) / starPoints)
            );
            const starFraction =
              (starAngle % ((2 * Math.PI) / starPoints)) /
              ((2 * Math.PI) / starPoints);

            const outerRadius = radius;
            const innerRadius = radius * 0.4;
            const currentRadius =
              starIndex % 2 === 0 ? outerRadius : innerRadius;
            ...
            const interpolatedRadius =
              radius1 + starFraction * (radius2 - radius1);
          ...
          default:
            ...
            const defaultCurrentAngle =
              -defaultAngleRad / 2 + progress * defaultAngleRad;
      ...
      >
        <Layer>
          ...
          {textLayers.map((textLayer) => {
            ...
            const shadowFilter =
              textLayer.textShadow.blur > 0 ||
              textLayer.textShadow.offsetX !== 0 ||
              textLayer.textShadow.offsetY !== 0
                ? {
                ...
                onDragEnd={(e) => {
                  ...
                  if (
                    ...
                    try {
                      ...
                      console.error(
                        "Error updating text layer position:",
                        error
                      );
                    }
                  }
                ...
                onTransformEnd={(e) => {
                  ...
                  if (
                    ...
                    try {
                      ...
                      console.error(
                        "Error updating text layer transform:",
                        error
                      );
                    }
                  }
