...
const KonvaCanvas = dynamic(
  () => import("./KonvaCanvas").then((mod) => ({ default: mod.default })),
  {
    ssr: false,
    loading: () => (
...
const fontSizes = [
  8, 10, 12, 14, 16, 18, 20, 24, 28, 32, 36, 40, 48, 56, 64, 72, 96, 120, 144,
  180,
];
...
export default function TextEditor({
  ...
  const selectedText = textLayers.find(
    (layer) => layer.id === Array.from(selectedIds)[0]
  );
  const selectedLayers = textLayers.filter((layer) =>
    ...
    try {
      ...
      if (savedData) {
        ...
        if (
          ...
          console.log(
            "Saved data doesn't match current image, clearing localStorage"
          );
  ...
  const updateTextStyle = useCallback(
    (property: keyof TextStyle, value: string | number) => {
      console.log("updateTextStyle called:", property, value);

      if (hasSelection) {
        const newLayers = textLayers.map((layer) =>
          selectedIds.has(layer.id) ? { ...layer, [property]: value } : layer
        );
        setTextLayers(newLayers);
        addToHistory(newLayers);
      }
  ...
  const updateTextShadow = useCallback(
    (property: keyof TextStyle["textShadow"], value: string | number) => {
      setTextStyle((prev) => ({
        ...prev,
  ...
  const deleteText = useCallback(() => {
    if (hasSelection) {
      const newLayers = textLayers.filter(
  ...
  const moveLayerUp = useCallback(() => {
    if (hasSelection && selectedIds.size === 1) {
      ...
      const currentIndex = textLayers.findIndex(
  ...
  const bringToFront = useCallback(() => {
    if (hasSelection && selectedIds.size === 1) {
      ...
      if (selectedLayer) {
        const otherLayers = textLayers.filter(
          (layer) => layer.id !== selectedId
        );
  ...
  const toggleLock = useCallback(() => {
    if (hasSelection) {
      ...
      if (selectedLayers.length > 0) {
        ...
        const newLayers = textLayers.map((layer) =>
          selectedIds.has(layer.id) ? { ...layer, locked: newLockState } : layer
        );
        setTextLayers(newLayers);
        addToHistory(newLayers);
      }
  ...
  const updateWarpProperty = useCallback(
    (property: "warpRadius" | "warpAngle" | "warpSpacing", value: number) => {
      if (hasSelection) {
        const newLayers = textLayers.map((layer) =>
          selectedIds.has(layer.id) ? { ...layer, [property]: value } : layer
        );
          ...
          switch (pathType) {
            ...
            case "wave":
              ...
              for (let i = 0; i < waveCount; i++) {
                const waveX =
                  centerX + (i - waveCount / 2) * ((radius * 2) / waveCount);
                const waveY =
                  centerY + (i % 2 === 0 ? -waveRadius : waveRadius);
                wavePath += ` Q ${waveX} ${waveY} ${
                  waveX + (radius * 2) / waveCount
                } ${centerY}`;
              }
              path = wavePath;
              break;
            ...
            case "zigzag":
              ...
              for (let i = 1; i <= zigzagCount; i++) {
                ...
                const y =
                  centerY + (i % 2 === 0 ? zigzagHeight : -zigzagHeight);
                zigzagPath += ` L ${x} ${y}`;
              }
  ...
  const updateSpacingHintColor = useCallback(
    (color: string) => {
      if (hasSelection) {
        const newLayers = textLayers.map((layer) =>
          selectedIds.has(layer.id)
  ...
  const updateSpacingHintOpacity = useCallback(
    (opacity: number) => {
      if (hasSelection) {
        const newLayers = textLayers.map((layer) =>
          selectedIds.has(layer.id)
  ...
  const alignLayersVertically = useCallback(() => {
    if (selectedIds.size >= 2) {
      ...
      const centerY =
        selectedLayers.reduce((sum, layer) => sum + layer.y, 0) /
        selectedLayers.length;

      const newLayers = textLayers.map((layer) =>
        selectedIds.has(layer.id) ? { ...layer, y: centerY } : layer
      );
      setTextLayers(newLayers);
      addToHistory(newLayers);
    }
  ...
  const alignLayersHorizontally = useCallback(() => {
    if (selectedIds.size >= 2) {
      ...
      const centerX =
        selectedLayers.reduce((sum, layer) => sum + layer.x, 0) /
        selectedLayers.length;

      const newLayers = textLayers.map((layer) =>
        selectedIds.has(layer.id) ? { ...layer, x: centerX } : layer
      );
      setTextLayers(newLayers);
      addToHistory(newLayers);
    }
    ...
    return new Promise<{ name: string; url: string; type: string }>(
      (resolve, reject) => {
        ...
        if (!isValidType) {
          reject(
            new Error(
              "Invalid font file type. Please upload TTF, OTF, WOFF, or WOFF2 files."
            )
          );
          return;
        }
  ...
  return (
    <div
      className={`flex flex-col lg:flex-row h-screen bg-gray-100 ${className}`}
    >
      {/* Canvas Area */}
            ...
            {isMultiSelect && (
              <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                ...
                <div className="text-xs text-blue-700 mb-2">
                  {selectedIds.size} layer{selectedIds.size !== 1 ? "s" : ""}{" "}
                  selected
                </div>
                <div className="text-xs text-blue-600 bg-blue-100 p-2 rounded">
                  <strong>Tip:</strong> Hold Ctrl/Cmd + Click to select multiple
                  layers
                </div>
              </div>
            )}
              ...
              <>
                ...
                <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                  ...
                  <div className="mb-4">
                    ...
                    >
                      <optgroup label="System Fonts">
                        ...
                        <option value="Bookman Old Style">
                          Bookman Old Style
                        </option>
                      ...
                      <div className="mt-3">
                        ...
                        <div className="space-y-1">
                          {customFonts.map((font) => (
                            <div
                              key={font.name}
                              className="flex items-center justify-between bg-gray-100 p-2 rounded text-xs"
                            >
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-500 mt-1">
                      Current: {selectedText.fontWeight} | Font:{" "}
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        updateTextStyle("stroke", e.target.value)
                      }
                      className="w-12 h-10 border border-gray-300 rounded-md cursor-pointer"
                    />
                    <input
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateTextStyle("opacity", Number(e.target.value))
                    }
                    className="w-full"
                  />
                  <div className="flex justify-between text-xs text-gray-500 mt-1">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateTextStyle("lineHeight", Number(e.target.value))
                    }
                    className="w-full"
                  />
                  <div className="flex justify-between text-xs text-gray-500 mt-1">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateTextStyle("letterSpacing", Number(e.target.value))
                    }
                    className="w-full"
                  />
                  <div className="flex justify-between text-xs text-gray-500 mt-1">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateTextStyle("textDecoration", e.target.value)
                    }
                    className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent text-black"
                  >
                ...
                <div>
                  ...
                  <div className="mb-3">
                    <label className="block text-xs text-gray-600 mb-1">
                      Blur: {selectedText.textShadow.blur}px
                    </label>
                    <input
                      type="range"
                      min="0"
                      ...
                      onChange={(e) =>
                        updateTextShadow("blur", Number(e.target.value))
                      }
                      className="w-full"
                    />
                  </div>
                  ...
                  <div className="mb-3">
                    <label className="block text-xs text-gray-600 mb-1">
                      Offset X: {selectedText.textShadow.offsetX}px
                    </label>
                    <input
                      type="range"
                      min="-20"
                      ...
                      onChange={(e) =>
                        updateTextShadow("offsetX", Number(e.target.value))
                      }
                      className="w-full"
                    />
                  </div>
                  ...
                  <div className="mb-3">
                    <label className="block text-xs text-gray-600 mb-1">
                      Offset Y: {selectedText.textShadow.offsetY}px
                    </label>
                    <input
                      type="range"
                      min="-20"
                      ...
                      onChange={(e) =>
                        updateTextShadow("offsetY", Number(e.target.value))
                      }
                      className="w-full"
                    />
                  </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateTextStyle("paragraphWidth", Number(e.target.value))
                    }
                    className="w-full"
                  />
                  <div className="flex justify-between text-xs text-gray-500 mt-1">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-500 w-16 text-right">
                      {selectedText.rotation.toFixed(2)}°
                    </span>
                  </div>
                </div>
                ...
                <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="text-xs text-blue-700 bg-blue-100 p-2 rounded">
                      <strong>Auto-saves every 2 seconds</strong>
                      <br />
                ...
                <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                  ...
                  {selectedText?.isWarped && (
                    <>
                      ...
                      <div className="mb-3">
                        <label className="block text-xs font-medium text-green-700 mb-2">
                          Radius: {selectedText.warpRadius}px
                        </label>
                        <input
                          type="range"
                          min="50"
                      ...
                      <div className="mb-3">
                        <label className="block text-xs font-medium text-green-700 mb-2">
                          Spacing: {selectedText.warpSpacing}
                        </label>
                        <input
                          type="range"
                          min="0.5"
                ...
                <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-200">
                  ...
                  <div className="mb-3">
                    <label className="block text-xs font-medium text-indigo-700 mb-2">
                      Hint Opacity:{" "}
                      {Math.round(
                        (selectedText?.spacingHintOpacity || 0.6) * 100
                      )}
                      %
                    </label>
                    <input
                      ...
                      onChange={(e) =>
                        updateSpacingHintOpacity(Number(e.target.value))
                      }
                      className="w-full"
                    />
                  </div>
                  ...
                  {isMultiSelect && (
                    <div className="text-xs text-indigo-600 bg-indigo-100 p-2 rounded">
                      <strong>Tip:</strong> Use spacing hints to visualize gaps
                      between layers
                    </div>
                  )}
                </div>
                    ...
                    >
                      ...
                      {selectedIds.size === 1
                        ...
                        : (() => {
                            ...
                            const allLocked = selectedLayers.every(
                              (layer) => layer.locked
                            );
                            return allLocked
                              ? `Unlock All (${selectedIds.size})`
                              : `Lock All (${selectedIds.size})`;
                          })()}
                    </button>
                    <button
